            ...
            for property in bc['properties']:
                ...
                if data_element_concept_id == property_code:
                    ...
                    if codelist_concept_id:
                        ...
                        if codelist_concept_id not in datasets_dict[dataset_name][variable_name]:
                            datasets_dict[dataset_name][variable_name][codelist_concept_id] = []

                        # Add submissionValue values and remove duplicates if any
                        datasets_dict[dataset_name][variable_name][codelist_concept_id] = list(
                            set(datasets_dict[dataset_name][variable_name][codelist_concept_id] + response_codes)
                            )
...
def update_datasets_dict():
    ...
    for (variable, dataset, codelist_concept_id), values in variable_values.items():
        ...
        datasets_dict[dataset][variable][codelist_concept_id] = sorted(
            list(set(datasets_dict[dataset][variable][codelist_concept_id]).union(values))
            )
...
def populate_study_elements():
    ...
    """
    # StudyName Element
    expression_studyname = Jsonata(f'study.versions["{studyversion}"].titles[type.decode = "Study Acronym"].text')
    template['Study']['StudyName'] = expression_studyname.evaluate(usdm_data)

    # StudyDescription Element
    expression_studydescription = Jsonata(f'study.versions["{studyversion}"].titles[type.decode = "Official Study Title"].text')
    template['Study']['StudyDescription'] = expression_studydescription.evaluate(usdm_data)

    # ProtocolName Element
    expression_protocolname = Jsonata(f'study.versions["{studyversion}"].titles[type.decode = "Study Acronym"].text')
...
def process_datasets():
    ...
    for dataset in datasets_dict:
        ...
        dataset_data = client.get_api_json(f"/mdr/sdtmig/{sdtmig.replace('.', '-')}/datasets/{dataset}")

        # Add information to Datasets
        template['Datasets'][dataset]['Description'] = dataset_data['label']
        template['Datasets'][dataset]['Class'] = dataset_data['_links']['parentClass']['title']
        ...
        for var in (v for v in dataset_data['datasetVariables'] if v['name'] in variable_list or v['core'] in ['Req', 'Exp']):
            ...
            if '_links' in var and 'codelist' in var['_links']:
                for codelist_item in var['_links']['codelist']:
                    if 'href' in codelist_item and codelist_item['href'] is not None:
                        ...
                        codelist_data = client.get_api_json(f"/mdr/ct/packages/sdtmct-{sdtmct}/codelists/{codelist_id}")
            ...
            if var['name'] in variable_list:
                process_variable_codelist(var, dataset, datasets_dict[dataset][var['name']])
...
def process_variable_codelist(var, dataset, restriction_codes=None):
    ...
    for codelist_item in var['_links']['codelist']:
        ...
        elif var['name'].endswith('TEST') and dataset in test_dict and var['name'] in test_dict[dataset]:
